...
const PricingPage = () => {
    ...
    try {
      ...
      const validPackages = packagesList
        .filter((pkg: Package) => {
          ...
          const hasPrice = pkg.currentPrice !== undefined || pkg.price !== undefined;
          const hasCredits = pkg.includedCredits !== undefined || pkg.credits !== undefined;
          return pkg && pkg.isActive !== false && pkg._id && pkg.name && hasPrice && hasCredits;
        })
        .map((pkg: Package) => ({
          ...pkg,
          // Normalize to new schema with fallback to legacy
          includedCredits: pkg.includedCredits ?? pkg.credits ?? 0,
          currentPrice: pkg.currentPrice ?? pkg.price ?? 0,
          actualPrice: pkg.actualPrice ?? pkg.price ?? pkg.currentPrice ?? 0,
          sortOrder: pkg.sortOrder ?? 0,
          features: pkg.features || [],
        }))
        .sort((a: Package, b: Package) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0)); // Sort by sortOrder
  ...
  const handleBuyPackage = (pkg?: Package) => {
    ...
    if (!authAPI.isAuthenticated()) {
      const returnTo = pkg ? `/payment?packageId=${pkg._id}` : "/payment";
      toast.error("Please sign in to purchase a package");
      navigate("/auth/sign-in", { state: { returnTo } });
      return;
    }

    // Navigate to payment page with package ID if available
    if (pkg) {
      navigate(`/payment?packageId=${pkg._id}`);
    } else {
      // For static pricing, navigate to payment page anyway
      // The payment page will handle the case when no package is available
      navigate("/payment");
    }
  };
  ...
  const handleUpdatePackage = async () => {
    ...
    try {
      ...
      await adminAPI.updatePackage(editingPackage._id, {
        ...
        includedCredits: parseInt(formData.includedCredits),
  ...
  const openEditDialog = (pkg: Package) => {
    ...
    setFormData({
      ...
      includedCredits: (pkg.includedCredits ?? pkg.credits ?? 0).toString(),
      actualPrice: (pkg.actualPrice ?? pkg.price ?? 0).toString(),
      currentPrice: (pkg.currentPrice ?? pkg.price ?? 0).toString(),
      offer: pkg.offer || "",
                    ...
                    <div className="flex items-baseline gap-1 mb-1">
                      {selectedPackage.offer && (
                        <div className="absolute -top-1 right-2">
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 rounded-full">
                            {selectedPackage.offer}
                          </span>
                        </div>
                      )}
                      <div className="flex items-baseline gap-2">
                        {selectedPackage.actualPrice > selectedPackage.currentPrice && (
                          <span className="text-lg font-medium text-violet-300/60 line-through">
                            ₹{selectedPackage.actualPrice}
                          </span>
                        )}
                        <span className="text-2xl font-bold text-white">₹{selectedPackage.currentPrice}</span>
                      </div>
                    </div>
                    ...
                    {packages.length > 1 && (
                      <div className="mb-3">
                        ...
                        <div className="flex justify-between mt-0.5 text-[10px] text-violet-300/60">
                          <span>₹{packages[0]?.currentPrice || 0}</span>
                          <span>₹{packages[packages.length - 1]?.currentPrice || 0}</span>
                        </div>
                      </div>
                    )}
                    <Button
          ...
          <DialogContent className="bg-black border-violet-400/20 text-white max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 mt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="edit-includedCredits">Included Credits *</Label>
                  <Input
                    id="edit-includedCredits"
                    type="number"
                    value={formData.includedCredits}
                    onChange={(e) => setFormData({ ...formData, includedCredits: e.target.value })}
                    className="bg-black/50 border-violet-400/20 text-white"
                  />
                </div>
                <div>
              ...
              <div>
                <Label htmlFor="edit-offer">Offer Badge (optional)</Label>
                <Input
                  id="edit-offer"
              ...
              <div className="flex gap-2 pt-4">
                <Button
                  ...
                  disabled={!formData.name || !formData.includedCredits || !formData.actualPrice || !formData.currentPrice}
                >
                  Update Package
                </Button>
                <Button
